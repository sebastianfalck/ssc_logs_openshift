...
""")
        ...
        if tipo == 'logs':
            ...
            html.write('<div class="log-title">📦 Despliegue YAML</div>')
            ...
            html.write('<div class="log-title">📊 Cuotas de Recursos</div>')
            for line in lines:
                esc = line.strip()
                if re.search(r'(used|hard):', esc):
                    html.write(f'<div class="line info">{esc}</div>\n')
                elif '100%' in esc:
                    html.write(f'<div class="line error">⚠️ {esc}</div>\n')
                else:
                    html.write(f'<div class="line normal">{esc}</div>\n')
